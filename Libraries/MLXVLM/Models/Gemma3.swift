...
// Previous parts of Gemma3.swift ... (Configurations, Model, etc.)
// Assume DefaultMessageGenerator is available from MLXLMCommon or a similar module.
// If not, you'd need to include its definition or a similar MessageGenerator.

public class Gemma3Processor: UserInputProcessor {
    private let config: Gemma3ProcessorConfiguration
    private let tokenizer: any Tokenizer
    // Assuming DefaultMessageGenerator is accessible
    ...
    public init(_ config: Gemma3ProcessorConfiguration, tokenizer: any Tokenizer) {
        ...
        // If you create a Gemma3MultimodalMessageGenerator as discussed for list-based content,
        // you would instantiate that here instead.
    ...
    public func preprocess(images: [CIImage], processing: UserInput.Processing?) throws -> (
        ...
        // Assuming imageSize in config is the target for preprocessing
        processingOverride.resize = CGSize(width: config.imageSize, height: config.imageSize)
    ...
    public func prepare(input: UserInput) async throws -> LMInput {
        // Generate message dictionaries using the MessageGenerator
        // This converts UserInput.Prompt (text, messages, or chat) into [[String: Any]]
        let messagesForTemplate = messageGenerator.generate(from: input)

        print("DEBUG: Messages for template (roles): \(messagesForTemplate.map { $0["role"] ?? "nil" })")
        print("DEBUG: Full messages for template: \(messagesForTemplate)")

        // Tokenize the messages using the chat template
        // The Jinja template provided will enforce role alternation here.
        // If messagesForTemplate doesn't conform, this line will throw an error.
        let promptTokens: [Int]
        do {
            promptTokens = try tokenizer.applyChatTemplate(messages: messagesForTemplate)
        } catch {
            print("ERROR: Failed to apply chat template: \(error)")
            print("ERROR: Problematic message sequence used: \(messagesForTemplate)")
            // It's good practice to throw a more specific error or rethrow
            throw VLMError.processing("Chat template application failed. Ensure message roles alternate correctly (e.g., user, assistant, user...). Original error: \(error.localizedDescription)")
        }


        print("DEBUG: Prompt token IDs (after applyChatTemplate): \(promptTokens)")
        if let decoded = try? tokenizer.decode(tokens: promptTokens) {
            print("DEBUG: Decoded prompt tokens (after applyChatTemplate): \(decoded)")
        }
        ...
        if !input.images.isEmpty {
            ...
            // Use boiTokenId from Gemma3ProcessorConfiguration
            // This assumes config.imageTokenId (255999) is the beginning-of-image marker.
            let boiTokenId = self.config.imageTokenId
            print("DEBUG: Using BOI token ID: \(boiTokenId) for search in promptTokens.")
            ...
            print("DEBUG: Found BOI token indices: \(boiTokenIndices)")


            // Make sure we have the right number of image tokens found vs. actual images
            // This check depends on `boiTokenId` being correctly present in `promptTokens`
            // for each image that needs to be processed.
            guard boiTokenIndices.count == input.images.count else {
                throw VLMError.processing(
                    "Number of BOI tokens found in prompt (\(boiTokenIndices.count)) does not match number of images (\(input.images.count)). Ensure your input text/template correctly produces a BOI token (\(boiTokenId)) for each image."
                )
            }
            ...
            for boiIndex in boiTokenIndices {
                ...
                // Add newlines before the BOI token (Token 108 is specific, ensure it's correct for your tokenizer)
                // This formatting might be better handled by the chat template itself if it's a consistent requirement.
                result.append(108)

                result.append(boiTokenId)

                // Use imageSeqLength from Gemma3ProcessorConfiguration for the number of image patch tokens
                let imagePatchTokenId = 262144 // TODO: This should ideally come from config (e.g., Gemma3Configuration.imageTokenIndex)
                let numImagePatchTokens = self.config.imageSeqLength // This is mmTokensPerImage
                result.append(contentsOf: Array(repeating: imagePatchTokenId, count: numImagePatchTokens))

                let eoiTokenId = 256000  // TODO: This should ideally come from config (e.g., Gemma3Configuration.eoiTokenIndex)
            ...
            print("DEBUG: Final prompt token IDs (after VLM insertions): \(finalPromptTokens)")
            if let decodedFinal = try? tokenizer.decode(tokens: finalPromptTokens) {
                print("DEBUG: Decoded final prompt tokens (after VLM insertions): \(decodedFinal)")
            }
        }

        let promptArray = MLXArray(finalPromptTokens).expandedDimensions(axis: 0)
        let mask = ones(like: promptArray).asType(.int8) // Or another appropriate dtype
        return LMInput(
...
// The rest of your Gemma3.swift file (Gemma3ProcessorConfiguration, Gemma3 model, etc.)
// ...
